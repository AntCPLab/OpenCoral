...
#ifndef GC_VECTORPREP_H_
#define GC_VECTORPREP_H_
...
template<class T>
class VectorPrep : public BufferPrep<T>
{
...
public:
    ...
    VectorPrep(DataPositions& usage) :
            BufferPrep<T>(usage), part_prep(usage), part_proc(0)
    {
    }

    VectorPrep(SubProcessor<T>*, DataPositions& usage) :
            VectorPrep(usage)
    {
    }
