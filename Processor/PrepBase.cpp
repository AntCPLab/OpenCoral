...
string PrepBase::get_normal_filename(const string& prep_data_dir,
        const string& type_short, int my_num, int thread_num)
{
    return prep_data_dir + "normals-" + type_short + "-P"
            + to_string(my_num) + get_suffix(thread_num);
}
        ...
        size_t used, bool large)
{
    ...
    {
        cerr << "Significant amount (unused: " << n << ", used: " << used << ") of unused " << name << " of " << type_string
        ...
        int n_bits, size_t used, bool malicious)
{
    ...
    {
        cerr << "Significant amount (unused: " << n*n_batch << ", used: " << used << ") of unused edaBits of size " << n_bits
                << ". ";
        if (malicious)
            cerr << "This protocol has a large minimum batch size, "
                    << "which makes this unavoidable for small programs.";
